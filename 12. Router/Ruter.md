...
우리가 children을 썻을 때 부모와 자식은 서로 필요한 데이터가 다릅니다.

우리의 예제를 바탕으로 정리를 해 보자면 아래와 같습니다.

| URL          | 컴포넌트    | 필요 데이터                  |
| :----------- | :---------- | :--------------------------- |
| /            | `<Root>`    | 컨택트의 리스트 데이터       |
| contacts/:id | `<Contact>` | id에 따른 컨텍트 개별 데이터 |

즉 아래의 이미지 처럼 전체 데이터에서 특정 데이터만 필요하는 것을 알 수 있습니다.

[이미지](./Loading%20Data.png)

이런 상황일 때 하나의 큰 데이터에서 하위 컴포넌트에게 정보를 줄 때 저희는 라우터를 썻기 때문에 props를 이용하여 정보를 넘겨 줄 수 없습니다.
